...
icon, both windows' icon and the notification-area icon — one file, so they cannot drift.
It is generated by `make-icon.py` beside it, which draws a rounded square in the accent
gradient from `Palette.xaml` with a white W, thickening the stroke and dropping the padding
at 24 pixels and below. Change the script and re-run it rather than editing the binary:

```bash
cd src/WinSW.Gui/Assets && python3 make-icon.py
```
